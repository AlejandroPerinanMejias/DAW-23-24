class Monedero {
    nombre;
    b5;
    b10
    b20;
    constructor (nombre, b5, b10, b20) {
        this.nombre = nombre;
        this.b5 = b5;
        this.b10 = b10;
        this.b20 = b20;
    }
    getNombre() {
        return this.nombre;
    }
    getb5() {
        return this.b5;
    }
    getNb10() {
        return this.b10;
    }
    getb20() {
        return this.b20;
    }
    getTotal() {
        return (this.b5*5)+(this.b10*10)+(this.b20*20);
    }
    setNombre (nombre) {
        this.nombre = nombre;
    }
    setb5 (b5) {
        this.b5 = b5;
    }
    setb10 (b10) {
        this.b10 = b10;
    }
    setb20 (b20) {
        this.b20 = b20;
    }
    static mostDinero(m1, m2, m3, m4, m5, m6, m7) {
        let granMonedero;
        if (m1.getTotal() > m2.getTotal()) {
            granMonedero = m1;
        } else granMonedero = m2;
        if (granMonedero.getTotal() < m3.getTotal()) {
            granMonedero = m3;
        }
        else if (granMonedero.getTotal() < m4.getTotal()) {
            granMonedero = m4;
        }
        else if (granMonedero.getTotal() < m5.getTotal()) {
            granMonedero = m5;
        }
        else if (granMonedero.getTotal() < m6.getTotal()) {
            granMonedero = m6;
        }
        else if (granMonedero.getTotal() < m7.getTotal()) {
            granMonedero = m7;
        }
    }
}

let m1 = new Monedero("Monedero 1", 0, 0, 2);
let m2 = new Monedero("Monedero 2", 0, 4, 0);
let m3 = new Monedero("Monedero 3", 8, 1, 0);
let m4 = new Monedero("Monedero 4", 2, 0, 1);
let m5 = new Monedero("Monedero 5", 0, 2, 1);
let m6 = new Monedero("Monedero 6", 2, 2, 2);
let m7 = new Monedero("Monedero 7", 0, 3, 0);

console.log(Monedero.mostDinero(m1, m2, m3, m4, m5, m6, m7));